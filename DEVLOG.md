...
- 明确最终产品目标是网球约球/打球平台，而不是人格测试本身。
- 更新 README、README.zh.md、PROJECT_STATUS.md、CLAUDE.md 和 AGENTS.md，将当前"网球兵器谱"定位为冷启动入口、传播玩法和玩家风格标签沉淀模块。
- 在项目状态中补充后续路线：用户档案、约球发布、报名确认、打球反馈、球友匹配和社区传播。
- 明确暂未实现账号体系、场地/时间选择、约球生命周期、聊天通知、支付和真实场馆集成。

### 验证

- `just check-all`：ok（Rust fmt/clippy/nextest、前端 typecheck/lint、WASM 构建和 Webpack 生产构建均通过）

### 取舍

- 本次只修正产品北极星和范围边界，不提前实现约球功能，避免把未设计清楚的账号、场地、通知或支付能力半成品塞进当前 MVP。
