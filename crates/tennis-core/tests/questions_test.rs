...
fn test_question_ids_are_sequential() {
    ...
    for (i, q) in questions.iter().enumerate() {
        assert_eq!(q.id, i + 1, "question id should be sequential starting from 1");
    }
}
...
fn test_all_questions_have_non_empty_text() {
    for q in get_questions() {
        assert!(!q.text.is_empty(), "question {} should have non-empty text", q.id);
    }
}
...
fn test_all_options_have_non_empty_text() {
    for q in get_questions() {
        for (text, _) in &q.options {
            assert!(!text.is_empty(), "question {} should have non-empty option text", q.id);
        }
    }
}
...
fn test_option_answers_are_a_b_c_d() {
    ...
    for q in get_questions() {
        for (i, (_, answer)) in q.options.iter().enumerate() {
            assert_eq!(*answer, expected[i], "question {} option {} should map to {:?}", q.id, i, expected[i]);
        }
    }
}
